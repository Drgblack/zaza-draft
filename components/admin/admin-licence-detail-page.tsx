...
export function AdminLicenceDetailPageClient({ licenceId }: { licenceId: string }) {
  ...
  const loadData = useMemo(
    () => async () => {
      ...
      const response = await fetch(`/api/admin/licences/${licenceId}`, {
  ...
  const handleAssign = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/licences/${licenceId}/members`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleRemove = async (uid: string) => {
    try {
      ...
      const response = await fetch(`/api/admin/licences/${licenceId}/members/${uid}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      })
      if (!response.ok) {
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-6 px-4 py-10 sm:px-6">
      <header className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <AdminNav active="licences" canManageUsers={role === "super_admin"} />
      </header>
        ...
        <div className="flex flex-col gap-6">
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-2">
              <Input aria-label="School name" value={form.schoolName} disabled={!canManage} onChange={(event) => setForm((current) => ({ ...current, schoolName: event.target.value }))} />
              <Input aria-label="Contact email" value={form.contactEmail} disabled={!canManage} onChange={(event) => setForm((current) => ({ ...current, contactEmail: event.target.value }))} />
              <Input aria-label="Notes" value={form.notes} disabled={!canManage} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} />
              <Input aria-label="Seat limit" value={form.seatLimit} disabled={!canManage} onChange={(event) => setForm((current) => ({ ...current, seatLimit: event.target.value }))} />
              <select aria-label="Status" value={form.status} disabled={!canManage} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value as "trial" | "active" | "expired" | "cancelled" }))} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900">
                <option value="trial">trial</option>
                <option value="active">active</option>
                <option value="expired">expired</option>
                <option value="cancelled">cancelled</option>
              </select>
              <Input aria-label="Start date" type="date" value={form.startDate} disabled={!canManage} onChange={(event) => setForm((current) => ({ ...current, startDate: event.target.value }))} />
              <Input aria-label="End date" type="date" value={form.endDate} disabled={!canManage} onChange={(event) => setForm((current) => ({ ...current, endDate: event.target.value }))} />
            </div>
            {canManage ? (
              <div className="mt-4">
